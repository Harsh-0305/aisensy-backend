...
app.post("/webhook", async (req, res) => {
  try {
    ...
    let userMessage = req.body.data.message.message;
    ...
    if (buttonTitle !== "manage bookings") {
      ...
      if (packageNameMatch && expCodeMatch && dateMatch) {
        ...
        const { data: pkg1, error: pkgError1 } = await supabase.rpc(
          "check_date_in_start_date_2",
          {
        ...
        if (pkgError1) {
          ...
          return res
            .status(500)
            .json({ error: "Something went wrong checking the date" });
        }
        ...
        const packageNotFoundMessage =
          "We couldn't find any trips matching the provided details. Please double-check the information or explore available options at Tripuva.com";
        ...
        const messageSent1 = await sendWhatsAppMessage2(
          userPhone,
          " ",
          responseMessage1,
        );
        ...
        if (!messageSent1) {
          return res
            .status(500)
            .json({ error: "Failed to send WhatsApp message to the customer" });
        }
    ...
    const response = await axios.post(
      ...
      },
      {
        headers: {
          ...
          Authorization:
            "Basic dU9OWUI3bEtRbmFhM0t6VWViSWlBVVRyN09COG1sNENzdHVnalAtQkdBSTo=",
        },
      },
    );
...
async function processRazorpayWebhook(body, signature) {
  try {
    ...
    const packageName = body.payload.payment_link.entity.description.replace(
      "Payment for ",
      "",
    );
    ...
    if (event === "payment_link.paid" && status === "paid") {
      ...
      if (fetchPackageError || !packageData?.start_date_2) {
        ...
        if (
          startDateSlots[bookingPackageDate] !== undefined &&
          startDateSlots[bookingPackageDate] > 0
        ) {
          ...
          if (updateError) {
            ...
            console.log(
              `🛎️ Slot updated for ${bookingPackageDate}. Remaining slots: ${startDateSlots[bookingPackageDate]}`,
            );
          }
        } else {
          console.warn(
            `⚠️ No available slot to decrement for ${bookingPackageDate} or date not found`,
          );
        }
      }
      ...
      if (fetchError) {
        ...
        if (updateError) {
          ...
          console.log(
            `✅ Package "${packageName}" appended to user's booked_package`,
          );
        }
      }
    }
...
app.get("/health", (req, res) => {
  logger.info("Health check received at:", new Date().toISOString());
