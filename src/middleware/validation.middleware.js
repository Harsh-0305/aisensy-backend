...
export const validateWebhookData = (req, res, next) => {
    ...
    if (message.message) {
        ...
        if (messageText.includes('Trip:')) {
            ...
            const expCodeMatch = messageText.match(/\(?\s*Experience\s*code[:\s]*([A-Z0-9]+)\s*\)?/i);
            const dateMatch = messageText.match(/Trip\s*Date[:\s]*([0-9]{2}-[A-Za-z]{3}-[0-9]{2})/i);
...
export const validateRazorpayWebhook = (req, res, next) => {
    ...
    if (!event || !payload || !payload.payment_link || !payload.payment_link.entity) {
